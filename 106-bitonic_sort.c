...
#include <stdio.h>
/**
 * swap2 - change two values in ascending or descending order
 * @arr: array
 * @item1: item one
 * @item2: item two
 * @order: 1: ascending order, 0 descending order
 */
void swap2(int arr[], int item1, int item2, int order)
{
	int temp;
...
/**
 * merge2 - sort bitonic sequences recursively in both orders
 * @arr: array
 * @low: first element
 * @nelemnt: elements number
 * @order: 1: ascending order, 0 descending order
 */
void merge2(int arr[], int low, int nelemnt, int order)
{
	int mid, i;
	...
	if (nelemnt > 1)
	{
		...
		for (i = low; i < low + mid; i++)
			swap2(arr, i, i + mid, order);
		merge2(arr, low, mid, order);
		merge2(arr, low + mid, mid, order);
	}
}
/**
